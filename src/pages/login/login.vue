<template>
	<view class="login">
		...
		<view class="content">
			...
			<!-- 主体表单 -->
			<view class="main">
				<wInput v-model="email" type="text" maxlength="30" placeholder="邮箱" :focus="isFocus"></wInput>
				<wInput v-model="password" type="password" maxlength="30" placeholder="密码"></wInput>
			</view>
			<wButton class="wbutton" text="登 录" :rotate="isRotate" @click="startLogin"></wButton>
			<wButton class="wbutton" text="微信快捷登入" :rotate="isRotate" @click="wxLogin"></wButton>
			<!-- 其他登录 -->
...
<script>
	...
	export default {
		...
		methods: {
			...mapMutations({
			      setUser: 'setUser' ,// 将 `this.add()` 映射为 `this.$store.commit('increment')`
				  seIsLogin : 'seIsLogin'
			    }),
			wxLogin() {
				//获取登入code
				...
				this.getUserInfo(info => {
					...
					wxAuthorization(httpData).then((res) => {
