<template>
	<view class="container">
		...
		<view class="cate-section">
			<view class="cate-item" v-for="(item, index) in specialList" :key="index">
		...
		methods: {
			...
			countQSTabsWxsListHeight() {
				...
				view.exec(res => {
					...
					if (res[0] && res[0].height) {
						this.QSTabsWxsListHeight = res[0].height;
					}
			...
			async loadData() {
				...
				try {
					...
					this.titleNViewBackground = bannerList.data[0].background;
					this.swiperLength = bannerList.data.length;
					this.carouselList = bannerList.data;
					let specialList = await getSpecialList();
					this.specialList = specialList.data
					//获取tabs
					let tabslList = await getTabs();
					this.tabsList = tabslList.data
					this.countQSTabsWxsListHeight();
					this.$refs.QSTabsWxsList.setTabs(this.tabsList);
				} catch (err) {
					console.log(err)
	...
	.hot-floor {
		...
		.floor-img-box {
			...
			&:after {
				...
				width: 100%;
				height: 100%;
				background: linear-gradient(to right, rgba(255, 255, 255, .06) 30%, #f8f8f8);
			}
		}
