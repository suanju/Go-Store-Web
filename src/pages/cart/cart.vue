<template>
	<view class="container">
		...
		<view v-if="!isLogin || empty===true" class="empty">
			<image src="/static/img/illustrations/emptyCart.png" mode="aspectFit"></image>
		...
		<view v-else>
			...
			<transition-group name="list" tag="div" class="cart-list">
				...
				<block v-for="(item, index) in cartList" :key="item.id">
					<view class="cart-item animate__animated " :class="{'b-b': index!==cartList.length-1}">
						...
						<view class="item-right">
							...
							<view class="numberbox">
								<text class="numbertxt">数量:</text>
								<u-number-box v-model="item.number">
									<view slot="minus" class="minus" @click="modifyInventory('-',item.id)">
										<u-icon name="minus" size="12"></u-icon>
									</view>
									<text slot="input" style="width: 50px;text-align: center;"
										class="input">{{item.number}}</text>
									<view slot="plus" class="plus" @click="modifyInventory('+',item.id)">
...
<script>
	...
	export default {
		...
		methods: {
			//请求数据
			async loadData() {
				let list = await getShoppingCart();
			...
			deleteCartItem(index) {
				...
				try {
					...
					let list = delShoppingCart(httpData);
					this.$set(this.cartList[index], "animation", true)
					setTimeout(() => {
			...
			//修改购物车数量
			modifyInventory(type, id) {
				try {
					let httpData = {
						tp: type,
						id: id
					}
					let list = modifyInventory(httpData);
				} catch (err) {
					console.log(err)
				}
			},
